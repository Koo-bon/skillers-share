...
> 💡 **Claude Code(터미널)에서 가장 완전하게 돌아갑니다** — 대화 기록 자동 스캔 + 덱 + 인터넷 링크 배포까지. 터미널에서 `claude` 열고 `공유회 준비해줘`.
> **claude.ai 웹·앱에서도 쓸 수 있지만 제한 모드예요** — 대화 기록 **자동 스캔**과 **인터넷 링크 자동 배포**는 여러분 컴퓨터에서만 되는 기능이라 빠지고, 주제는 직접 알려주면 되고 덱은 HTML 파일로 나옵니다. (claude.ai에서 쓰려면 스킬을 그쪽에 따로 업로드해야 합니다 — 아래 참고)
